...
export default function VideoPlayer(
  { streamUrl, onStop, onEnded }: VideoPlayerProps,
) {
  ...
  useEffect(() => {
    ...
    try {
      if ((window as any).Hls?.isSupported()) {
        ...
        onVideoError(new Error("Your browser doesn't support HLS streaming."));
      }
    } catch (err) {
      onVideoError(err);
    }
      ...
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <div className="text-white text-xl flex flex-col items-center gap-4">
            <i className="ri-loader-4-line animate-spin text-5xl"></i>
            <p>Loading stream...</p>
          </div>
        </div>
      )}
...
function SvgButton({
  ...
  return (
    <button
      ref={buttonRef}
      onClick={onClick}
      className={`w-14 h-14 rounded-full text-xl flex items-center justify-center transition-all duration-200 ${
